...
const GetProductByPrice = () => {
    const [productPrice, setProductPrice] = useState({ minprice: 0, maxprice: 0 })
    ...
    const changeHandler = (c) => {
        setProductPrice({ ...productPrice, [c.target.name]: c.target.value })
    }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.get(`http://localhost:8085/api/v1/products/unitprice?min=${productPrice.minprice}&max=${productPrice.maxprice}`);
            ...
            <div style={{ backgroundColor: '#e7eaf6', textAlign: 'center', minHeight: '100vh', paddingBottom: '50px' }}>
                ...
                <Container>
                    ...
                    <form onSubmit={handleSubmit} style={{ marginBottom: '20px' }}>
                        <label htmlFor="productPrice" style={{ marginRight: '10px', fontWeight: 'bold' }}>Enter Product Minimum Price:</label>
                        <input
                            type="text"
                            className="input"
                            name='minprice'
                        ...
                        />
                        <label htmlFor="productPrice" style={{ marginRight: '10px', fontWeight: 'bold' }}>Enter Product Maximum Price:</label>
                        <input
                            type="text"
                            className="input"
                            name='maxprice'
